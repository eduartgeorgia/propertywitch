...
router.get("/rag/status", async (_req, res) => {
    try {
        const stats = (0, aiService_1.getRAGSystemStats)();
        const categories = (0, index_1.getCategories)();
        return res.json({
            status: "ok",
            stats,
...
router.post("/rag/initialize", async (_req, res) => {
    try {
        ...
        const stats = (0, index_1.getRAGStats)();
        return res.json({
            status: "ok",
...
router.post("/rag/query", async (req, res) => {
    ...
    try {
        const results = await (0, index_1.retrieveKnowledge)(query, topK);
        return res.json({
            query,
...
router.get("/rag/knowledge", (_req, res) => {
    try {
        const knowledge = (0, index_1.getAllKnowledge)();
        return res.json({
            count: knowledge.length,
            categories: (0, index_1.getCategories)(),
...
router.delete("/rag/clear", async (req, res) => {
    ...
    try {
        (0, index_1.clearRAGData)(collection);
        return res.json({
            status: "ok",
...
router.post("/rag/listings/search", async (req, res) => {
    ...
    try {
        ...
        const parsedCriteria = criteria || (0, index_1.parseSearchQuery)(query || "");
        // Log the parsed criteria for debugging
        console.log("[RAG Search] Query:", query);
        console.log("[RAG Search] Criteria:", JSON.stringify(parsedCriteria));
        // Search with filters
        const results = (0, index_1.searchListingsByCriteria)(parsedCriteria, limit);
        // Format results
        const formattedResults = results.map(({ listing, score }) => ({
            ...
            priceFormatted: listing.metadata.priceEur
                ? `€${listing.metadata.priceEur.toLocaleString()}`
                : "Price unknown",
...
router.get("/rag/listings", async (req, res) => {
    ...
    try {
        ...
        const allResults = (0, index_1.searchListingsByCriteria)({}, 10000);
        // Paginate
