...
exports.shouldUseAgent = shouldUseAgent;
...
- Source: ${listing.sourceSite}`;
        ...
        case "compare_listings": {
            ...
            const comparison = listings.map(l => `• ${l.title}: €${l.priceEur.toLocaleString()}, ${l.areaSqm || '?'} m², ${l.locationLabel}`).join('\n');
            return {
                success: true,
                result: listings,
        ...
        case "calculate_costs": {
            ...
            if (price <= 97064)
                ...
                imtRate = 0.06; // Single rate for high values
...
async function runAgent(userQuery, userLocation, maxSteps = 5) {
    ...
    if (!health.available) {
        ...
        const searchResult = await (0, searchService_1.runSearch)({ query: userQuery, userLocation });
        return {
