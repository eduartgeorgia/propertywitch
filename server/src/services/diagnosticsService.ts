export type AccessMethod = "API" | "SITEMAP" | "PUBLIC_HTML" | "BYOC" | "NONE";
...
const probeApi = async (policy: SitePolicy) => {
...
const probeSitemap = async (policy: SitePolicy) => {
  try {
...
const robotsAllows = async (policy: SitePolicy) => {
  try {
...
const probePublicHtml = async (policy: SitePolicy) => {
...
export const diagnoseSite = async (policy: SitePolicy): Promise<SiteDiagnostics> => {
