import type { ParsedQuery, PriceIntent } from "../domain/query";
import { guessCurrency } from "./currencyService";
...
export const parseUserQuery = (input: string): ParsedQuery => {
  const raw = input.trim();
  const lower = raw.toLowerCase();

  let priceIntent: PriceIntent = { type: "none" };
  ...
  let listingIntent: 'sale' | 'rent' | undefined;
