 ...
 */
const extractPropertyType = (query: string): string => {
  const lower = query.toLowerCase();
...
const performInteractiveSearch = async (page: any, context: SearchContext): Promise<boolean> => {
  ...
  const success = await navigateHumanLike(page, IDEALISTA_BASE);
  if (!success) {
 ...
 */
const applyPriceFilters = async (page: any, context: SearchContext): Promise<void> => {
    ...
    const rawListings = await page.evaluate(() => {
      ...
      items.forEach((item, index) => {
        ...
        try {
          ...
          const linkEl = item.querySelector("a.item-link, a[href*='/imovel/'], a[href*='/terreno/'], h3 a") as HTMLAnchorElement;
          const imgEl = item.querySelector("img, picture img, [data-ondemand-img]") as HTMLImageElement;
...
export const idealistaStealthAdapter: SiteAdapter = {
  siteId: "idealista",
  siteName: "Idealista",
